...
def results_n_uncertainty(forecasts, year, lower_bound=5, upper_bound=95, model_uncertainty=False, **uncertainty):
	mean_value = np.mean(forecasts, axis=0)

	if model_uncertainty:
		lw_value = mix_norm_ppf(lower_bound*0.01, forecasts, uncertainty['uncertainty'])
		up_value = mix_norm_ppf(upper_bound*0.01, forecasts, uncertainty['uncertainty'])
		print('|===== Minimum River Level in {} (considering models uncertianties) ====='.format(year))
		print(tabulate([[mean_value, lw_value, up_value]], headers=['Mean', 'Lower Bnd ({}%)'.format(lower_bound), 'Upper Bnd ({}%)'.format(upper_bound)], tablefmt='github'))
	else:
		lw_value = np.percentile(forecasts, lower_bound, axis=0)
		up_value = np.percentile(forecasts, upper_bound, axis=0)
		print('|===== Minimum River Level in {} ====='.format(year))
		print(tabulate([[mean_value, lw_value, up_value]], headers=['Mean', 'Lower Bnd ({}%)'.format(lower_bound), 'Upper Bnd ({}%)'.format(upper_bound)], tablefmt='github'))
